...
_MarkerZone = createMarker [(format ["Pieton%1",str(round (random 9999))]),(_this select 0)];
...
_NbPersonnels = round (_Taille / 50);
...
for "_i" from 1 to round _Ratio * (round (_Taille/50)) + 3 do {
  ...
  _HommeCivil = _Groupes createUnit [SelectRandom HlisteUniteCivilsArabes,_MarkerZone call bis_fnc_randomPosTrigger,[],5,"Form"];
