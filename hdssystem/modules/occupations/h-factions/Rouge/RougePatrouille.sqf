...
_NbPersonnels = round (_Taille/100);
...
for "_i" from 1 to round (_Ratio + (round random _Ratio)) do {
  ...
  if(surfaceIsWater _SpawnPosition) then {
    _Groupes= [_NomMarqueur call bis_fnc_randomPosTrigger,east,(configfile >>  "CfgGroups" >> "east" >> "OPF_F" >> "SpecOps" >> "OI_diverTeam_Boat")] call BIS_fnc_spawnGroup;
    [_Groupes,(_this select 0),_Taille] call bis_fnc_taskpatrol;
    _Groupes enableDynamicSimulation true;
  }else{
    _Groupes = [_NomMarqueur call bis_fnc_randomPosTrigger,east,_NbPersonnels] call BIS_fnc_spawnGroup;
    [_Groupes,(_this select 0),_Taille] call bis_fnc_taskpatrol;
    _Groupes enableDynamicSimulation true;
  };
...
/* _Condition = "this";
_QuandActiver = "
_VehiculeCree = [([getpos thistrigger,1500,1000,1,0,60,0] call BIS_fnc_findSafePos),180,(SelectRandom ['O_Heli_Light_02_unarmed_F']),east] call bis_fnc_spawnvehicle;
_Troupes = [([getpos thistrigger,1500,1000,1,0,60,0] call BIS_fnc_findSafePos),east,5] call BIS_fnc_spawnGroup;
